...
def update_vehicle_data(veh_id, data, step, time_step):
    ...
    if veh_id not in data.keys():
        ...
        # data[veh_id]['a'] = [] # Acceleration [m/s^2]
        data[veh_id]['Lane ID'] = [] # Lane id
        
    
    data[veh_id]['Time [s]'].append(step*time_step)
    
    try:
        data[veh_id]['v'].append(ls.vehicle.getSpeed(veh_id))
        # data[veh_id]['a'].append(ls.vehicle.getAcceleration(veh_id))
        data[veh_id]['lane_id'].append(ls.vehicle.getLaneID(veh_id))
        ...
        # data[veh_id]['a'].append(ls.vehicle.getAcceleration(veh_id[3:]))
        data[veh_id]['Lane ID'].append(ls.vehicle.getLaneID(veh_id[3:]))
        pos = ls.vehicle.getPosition(veh_id[3:])
        data[veh_id]['x [m]'].append(pos[0])
        data[veh_id]['y [m]'].append(pos[1])

def process_vehicle_data(veh_id, time_step):
    """
    Processes the vehicle data generated by the function 'simulate_traffic' so
    data from one vehicle is combined into one .pickle file instead of being
    split between multiple files. Also does the following processing:
        
        - Converts the data types to arrays.
        - Filters the velocity data with a moving average of a userdefined
        number of seconds.
    ...
    veh_id : string
        ID of the vehicle.
    time_step : float
        time_step used in the 'simulate_traffic' function.
    ma_len : int
        Length of moving average window in seconds.
    ...
    veh_dict['veh_id'] = veh_id
    
    # Convert to DataFrame and save as csv file
    
    veh_df = pd.DataFrame(veh_dict)
    
    veh_df.to_csv(f'simulated_trips/{veh_id}.csv', index=False)

    return None

def process_simulation_data(time_step, pbar_bool, lite_mode=False, rng_seed=None):
    """
    Wrapper function for running the 'process_vehicle_data' function in
    parallel.
    ...
    lite_mode : booelan
        Only processes 10% of the vehicles in order to save time if True.
    rng_seed :  int
        Seed used by the random number generator for picking 10% of the
        vehicles to process. Only relevant when lite_mode is True.
    ...
    if 'simulated_trips' not in os.listdir():
        print("\nMaking 'simulated_trips'")
        os.mkdir('simulated_trips')
    else:
        print("\nPurging 'simulated_trips'")
        
        dir_length = len((folder:=os.listdir('simulated_trips')))
        pbar = tqdm(total=dir_length, position=0, leave=True)
        if dir_length != 0:
            for file in folder:
                os.remove(f'simulated_trips/{file}')
                pbar.update()
    ...
    partial_func = partial(process_vehicle_data, time_step=time_step)
    
    if pbar_bool:
        pbar=tqdm(total=len(veh_ids), position=0, leave=True)
        for _ in pool.imap_unordered(partial_func, veh_ids):
            pbar.update()

    else:
        pool.imap_unordered(partial_func, veh_ids)
    
    pool.close()
    
    return None

def simulate_traffic(config_path, duration, time_step, pbar_bool, lite_mode=False, rng_seed=None):
    """
    Simulates the traffic from a SUMO config file and tracks the time and 
    velocity of all vehicles. The simulation saves vehicle data every hour in
    ...
    ls.start(["sumo", "-c", config_path, "--step-length", str(time_step)])
    print(f'\nStarted simulation with timedelta: {ls.simulation.getDeltaT()}s')
    n_steps = duration*3600*(1/time_step)
    ...
    while step < n_steps:
        ...
        try:
            for veh_id in veh_list:
                update_vehicle_data(veh_id, data, step, time_step)
        except IndexError:
            pass
        
        step += 1
        
        if step > 0 and step%(3600/time_step)==0:
            # For every 1 hour
            
            timeslot_index = int(step/(3600/time_step))
    ...
    process_simulation_data(time_step, pbar_bool, lite_mode, rng_seed)
